...
# the full ELASTIC campaign: 90 jobs = 9 misfits x 5 optimizers x illumination
# preconditioner {ON, OFF} (A/B), joint Vp + Vs + density, 180 m smoothed
# initials. Generate its OWN shared obs first, then the 90 jobs:
condor_submit hpc/condor/run.sub -a 'kind=genobs_elastic'   # ONCE, wait for it
condor_submit hpc/condor/elastic_full_das.sub               # 90 jobs
# rank + illumination A/B on deep recovery:
